...
auc_test_table %>%
  ...
  mutate_if(is.numeric, function(x) sprintf('%.4f', x)) %>%
  flextable::flextable() %>%
...
var_imp_all$importance %>%
  ...
  mutate(var2 = ifelse(str_starts(var2, 'spatial_'), 'geodemographic', var2)) %>%
  mutate(var2 = ifelse(str_starts(var2, 'topic_'), 'review_topic', var2)) %>%
  mutate(var2 = ifelse(str_starts(var2, 'review_'), 'perception', var2)) %>%
  mutate(var2 = ifelse(str_starts(var2, 'geolocation_'), 'geolocation', var2)) %>%
  mutate(var2 = ifelse(var2 %in% c('payment_value', 'sum_freight', 'no_items', 'prod_categories'),
                       'behavioural', var2)) %>%
  group_by(var2) %>%
  summarise(Overall = sum(Overall)) %>%
  mutate(Overall = range01(Overall)*100) %>%

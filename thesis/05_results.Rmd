...
- Logistic Regression is an interpretable model by design - one can simply look at the model coefficients and infer about strength and direction of particular feature on the final prediction. However, as shown in the previous sections it is inferior to XGBoost with regards to predictions quality.
- XGB is however a *black box model*, meaning that its structore is too complex to be directly.
- To be able to infer hyptheses about importances and direction of influence for model prediction, XAI tools have to be used.
- In this study I have used 2 (3) techniques of XAI, namely Variable Importance (VI) and Partial Dependence Profile (PDP).
- The first one can answer the question which variables (or categories of variables) influences the predictions the most. 
- While the second - what is the direction of this influence?
...
- Najlepszy model (czy ze zbinowanymi kategoriami produktów?? do zastanowienia)
- Model ze wszystkimi zmiennymi, zbinowane kategorie do podziału wg tego jak są opisywane w literaturze ALBO wg "setu" zmiennych analizowanych, oba to chyba za dużo
...
var_imp_all$importance %>%
  as_tibble(rownames = 'var') %>%
  mutate(var2 = ifelse(str_starts(var, 'prod_cat_'), 'prod_categories', var)) %>%
  # mutate(var2 = ifelse(str_starts(var2, 'spatial_'), 'geodemographic', var2)) %>%
  # mutate(var2 = ifelse(str_starts(var2, 'topic_'), 'review_topic', var2)) %>%
  group_by(var2) %>%
  summarise(Overall = sum(Overall)) %>%
  mutate(Overall = range01(Overall)*100) %>%
  as.data.frame() -> temp_df_imp
...
rownames(temp_df_imp) <- temp_df_imp$var2
temp_df_imp$var2 <- NULL
var_imp_all$importance <- temp_df_imp
plot(var_imp_all)
```


All categories of features at once: (TODO: poc VarImpPlot z ggplota, do zastanowienia czy uspójnić z resztą i w ogóle które zostawić)
...
Variable importance z modelu ze wszystkimi zmiennymi posegregowane po "setach" zmiennych tak jak zrobiłem model
...
TODO: Wcześniej było cateris paribus dla średniej obserwacji, a teraz jest Partial dependence profile - średnie cateris paribus dla wszystkich obserwacji. Trochę zmienia rezultaty w review score

```{r}

# model_profile(explainer = explain, variables = "review_score") %>%
# plot() +
#   scale_x_continuous(n.breaks=10,limits = c(1,5))
...
# model_profile(explainer = explain, variables = "payment_value") %>%
# plot() +
...
# model_profile(explainer = explain, variables = "no_items") %>%
#   plot()

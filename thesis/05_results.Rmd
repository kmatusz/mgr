...
- I have tested 2 hypotheses. (1) The amount of money spent on the first purchase positively influences customer probability of buying for the second time. (2) categories bought by the customer can influence the customer probability to stay with the company.
...
In this study I have analysed Logistic Regression and XGBoost algorithms for the task of churn prediction. Logistic Regression is an interpretable model by design - one can simply look at the model coefficients and infer about strength and direction of particular feature influencd on the final prediction. However, as shown in the previous sections it is inferior to XGBoost with regards to predictions quality. XGB is a *black box model*, meaning that its structure is too complex to be directly inspected. To be able to test hyptheses about importances and direction of influence for model prediction, XAI tools have to be used.

In this study I have used 2 techniques of XAI, namely Variable Importance (VI) and Partial Dependence Profile (PDP). The first one can answer the question "which variables (or categories of variables) influences the predictions the most?", while the second - "What is the direction and strength of this influence?" 

#### Influence of features groups on customer churn (Variable importances)

In this study I have used Permutation Feature importance method. At first, performance score (in this case AUC) is computed on the whole dataset. Then, each of the variables is randomly permuted one-by-one, and predictions are made using such "blinded" variable. Later, the drop in model performance for each of the blinded features serves as its importance. TODO: cytowanie

I have examined variable importances for two of the XGB models from this study - one for the best set of variables (with included product categories), and another - with all variables included. The first one served ... (TODO: po co go opisywać w ogóle? Poza inspekcją modelu).

The latter one can help to check, which of the sets of variables from all of the tested ones were significant for model predictions.
...
gridExtra::grid.arrange(pl_varimp_raw, pl_varimp_binned, ncol=2)
```

On the left of figure \@ref(fig:varimp-categories) the unbinned variables importances from XGB model for the best variables (with included product categories) are presented. The values on the plot are scaled - meaning that the variable with the score 100 had the biggest influence, while with 0 - the smallest.
...
All of the dummies indicating product categories were in the later part of the ranking. One could wonder, why despite these features are relatively unimportant variables, they lead to 2.5% gain in AUC compared to the model without them.
This is because of the fact that conceptually all of the dummies indicating product categories encode one information, these variables' importances should be treated jointly. The same can be argued about geographic coordinates. To account for this, I have simply summarised variables' importances to bin them into 2 variables sets "geolocation" and "prod_categories". This information is presented on the right subfigure. 
After this operation, it can be seen that product categories gained in relative importance - now they are 4th variable. Also, geolocation variables set became a more important than payment value on its own.

TODO: Doszlifować wykres, teraz brzydko się najeżdżają etykiety kategorii
...
On figure \@ref(fig:varimp-all) variables' categories importances for the model with all variables are presented. I have grouped the variables into 3 categories:

- variables describing first transaction of the customer - payment value, product categories etc.
- variables describing perception - namely 1-5 review and dummies for topic of the textual review
- geographic variables - with 3 subgroups:
  - variables describing demographics of the region that the customer is in 
  - raw location - simply lon/lat coordinates
  - density - variable indicating whether the customer is in a densely-populated area.
  
The best set of variables is the one containing behavioural features. Next 2 sets of variables, namely geodemographic and spatial location have similar influence. The lowest impact on the model predictions have perception variables and density population indicator.
...
- 1-5 review of the purchase
- topic of the textual review
...
#### Direction and strength of features influence on customer churn (Partial Dependence Plots)

Partial Dependence Profile is based on Cateris Paribus technique. This technique is meant to perform "what-if" analysis for one observation x and one feature f. For this observation, the variable of interest is changed and the model predicts the response for each of these changes. Partial Dependence Profile is simply averaged value of such Cateris Paribus analysis for each of the observations from the dataset. 
...
profile_payment_value %>%
  ...
  geom_line(size = 0.7) +
  scale_x_continuous(n.breaks=10,limits = c(0,500)) + 
  geom_smooth(aes(y = `_yhat_`), alpha=0) +
  theme_minimal() +
  labs(
    title=NULL, 
    subtitle = NULL, 
  ...
  geom_vline(data = quantiles_payment, aes(xintercept = value), color = 'red') +
  geom_text(data = quantiles_payment, aes(label = a, x = value, y = 0),angle = 90, nudge_x = -10)

```

To facilitate drawing conclusions from PDP plot for payment value I have included smoothing line used *loess* technique. Also, I have added vertical red ?? lines indicating quantiles of the this variable's values in the dataset.

Model response for payment value is non-monotonous.  From an analysis of smoothed model response one can say that it is increasing to the point of around 100. This value is a median payment value in the whole dataset. After this threshold the probability to buy second time is falling slowly. 
...
Relationship betwen the number of items bought in the first purchase and the probability of the second purchase is in general negative. However, the model response stays roughly the same for 1 and 2 items. The most abrupt drop in model response is visible from 2 to 6 items - the model response drops 4-fold. Later the response is relatively stable.

For CRM, information about such relationship can lead to a following trade off. The more the customer buys in the first purchase, the bigger are the chances that they will not make a second purchase. This can have implications in cross-selling campaigns. The company can try to maximize the revenue from the first transaction by making the customer buy more, buy then there is bigger possibility that the customer will not make the second purchase. 

<!-- In previous studies the number of items bought per purchase for the customer was not analyzed. -->
...
In the case of geolocation data, I have created 2-d partial dependence estimates and visualized it on the figure ... . To facilitate the analysis I have marked 10 most populated Brazilian cities. It can be seen that the predictions are the highest in two distinct large spots - one having center close to Brasilia (new capital of the country), and the other one same latitude, but closer to the western country border. 
...
It can be seen that the predictions form a very visible pattern in stripes. As was noticed by @behrens2018spatial, it comes from the limitation of the model underlying XGBoost method, that is decision trees. The vanilla decision tree algorithm works by partitioning the feature space on a discrete basis, and a typical output of that model on 2-d space are visible rectangles. And as XGBoost is simply stacked decision trees, the resulting partition pattern is a bit more complex, but still decision-tree-typical artifacts are visible. 
 ...
 <!-- They are mostly located along the coast, with the exception of Brasilia (new capital of the country) and Manaus (the biggest city in Amazonia region). -->
...
Analysis of model responses in the case of review score should be treated with caution, as the analysis of variable importance shown it to be a relatively non-important variable. However, I have still tried to analyze PDP plots, to check the expectation of this relationship being positive. One the plot .. such PDP is presented. The model response is relatively flat in reaction to changes in review score. For reviews 1 and 2 the response is not changing at all - meaning that it doesn't matter *how bad* the review is. Rather, that unsatisfied customers will not buy again in general. From 2 to 5 the model response expectedly monotonically increases. 

From analysis of both variable importance plot and PDP plot one can conclude that customer satisfaction positively influences customer's propensity to buy again. However, the strength of this relationship is very week compared to other variables present in the model.

<!-- However, a unexpected behaviour can be noticed when the tested review changes from 1 to 2 - the model response drops. This means that not the customers with the most negative reviews have the biggest probability to buy second time but rather the ones with "moderately-negative". -->

...
knitr::opts_chunk$set(message = F, warning = F, echo=F, fig.width = 6, fig.height = 4, cache = T)
```
...
TODO: Description of the table, that only one model produced meaningful results  

```{r}
DT::datatable(read_csv(here('topics_table_to_paper.csv')) %>% mutate(`Example reviews` = str_replace(`Example reviews`, '\\n', '||||')))
```


### DBSCAN for long/lat data (TODO)

TODO: Opis wynikowych klastrów, może mapka jakie wyniki
...
I have also tried running the clustering with the epsilon obtained from using the elbow method, namely 2. Only 2 clusters were created, of which one of them contaied 99.97% of observations. The probable reason for that is that 2 would roughly translate to 500 kilometers of cluster range, which is way to big to model a way more fine-grained information about the population density on a country-level.
...
ggplot(pca_importances2, aes(x=PC, y=value)) +
  geom_line() +

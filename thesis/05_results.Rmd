...
On the left of figure \@ref(fig:varimp-categories) the unbinned variables importances from the XGB model for the best variables (with included product categories) are presented. The values on the plot are scaled - meaning that the variable with the score of 100 had the biggest influence, while 0 - the smallest.
...
All of the dummies indicating product categories were in the latter part of the ranking. One could wonder, why despite these features are relatively unimportant variables, they lead to a 2.5% gain in AUC compared to the model without them.
...
After this operation, it can be seen that product categories gained in relative importance - now they are the 4th variable. Also, geolocation variables set became more important than payment value on its own.
...
The best set of variables is the one containing behavioral features. The next 2 sets of variables, namely geodemographic and spatial location have a similar influence. The lowest impact on the model predictions have perception variables and density population indicator.
...
Partial Dependence Profile is based on the Cateris Paribus technique. This technique is meant to perform a "what if" analysis for one observation x and one feature f. For this observation, the variable of interest is changed and the model predicts the response for each of these changes. Partial Dependence Profile is simply averaged value of such Cateris Paribus analysis for each of the observations from the dataset. 
...
To facilitate drawing conclusions from the PDP plot for payment value I have included a smoothing line using *loess* technique. Also, I have added vertical red ?? lines indicating quantiles of this variable's values in the dataset.

Model response for payment value is non-monotonous.  From an analysis of smoothed model response, one can say that it is increasing to the point of around 100. This value is a median payment value in the whole dataset. After this threshold, the probability to buy for the second time is falling slowly. 
...
The relationship between the number of items bought in the first purchase and the probability of the second purchase is in general negative. However, the model response stays roughly the same for 1 and 2 items. The most abrupt drop in model response is visible from 2 to 6 items - the model response drops 4-fold. Later the response is relatively stable.
...
In the case of geolocation data, I have created 2-d partial dependence estimate and visualized it on the figure ... . To facilitate the analysis I have marked the 10 most populated Brazilian cities. It can be seen that the predictions are the highest in two distinct large spots - one having its center close to Brasilia (new capital of the country), and the other one same latitude, but closer to the western country border. 

It can be seen that the predictions form a very visible pattern in stripes. As was noticed by @behrens2018spatial, it comes from the limitation of the model underlying the XGBoost method, that is decision trees. The vanilla decision tree algorithm works by partitioning the feature space on a discrete basis, and typical output of that model on 2-d space are visible rectangles. And as XGBoost is simply stacked decision trees, the resulting partition pattern is a bit more complex, but still, decision-tree-typical artifacts are visible. 

 <!-- They are mostly located along the coast, with the exception of Brasilia (new capital of the country) and Manaus (the biggest city in the Amazonia region). -->
...
From the analysis of both the variable's importance plot and the PDP plot, one can conclude that customer satisfaction positively influences customer's propensity to buy again. However, the strength of this relationship is very week compared to other variables present in the model.

<!-- However, an unexpected behaviour can be noticed when the tested review changes from 1 to 2 - the model response drops. This means that not the customers with the most negative reviews have the biggest probability to buy for the second time but rather the ones with "moderately-negative". -->

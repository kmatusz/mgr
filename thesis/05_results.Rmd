...
- DBSCAN on lon/lat, PCA on demographic
  ...
  - ?? przenieść do methods description? bo bardzo krótkie i niewiele wnoszące rezultaty

After-modeling

- Model performance- AUC values for all models
  - methodological:
...
- Model performance- Bootstrap, K-S test, density plots
...
- Model performance- Lift curve, lift analysis
  ...
  - CRM: a way to enhance targeting strategies (which customers should be more/less targeted based on apriori probability of buying second time), a more meaningful baseline to assessing A/B testing strategies (probability to buy from the model vs. share of customers that bought)
  - Customer Lifetime Value for new customers - probability to buy second time from the model \* average buy second time 
  - Model performs better for almost all the cohorts of probabilities
  - segmentation+targeting - a way to reduce customer aquisition cost
  - Segmenation based on probability to buy second time
...
- Methodology - że może poprawiać przewidywania modelu, 
...
- TODO: cost analysis vs. profitability

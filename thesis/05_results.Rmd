...
# files_names_models <- files_names_models[c(1,7)]
...
<!-- TODO: nie wiem gdzie powinny być te sekcje - w rezultatach czy jednak od razu w opisie metod od razu pod poszczególnymi opisami -->
...
Cumulative variance explained by each of the consecutive loadings is presented on the plot \@ref(fig:pca-components). I have decided to leave the 10 most informative PCA eigenvectors. These account for 97.3% of the explained variance. 

```{r pca-components, fig.width = 3, fig.height = 2, fig.cap='Explained cumulative variance by each of the PCA loadings.'}
...
auc_test_table %>%
  ...
  rename(
    ...
    `AUC test` = AUC_test,
    `AUC train` = AUC_train,
    `Performance drop vs. \nthe best model` = AUC_perc_performance_drop
  ) %>%
flextable::flextable() %>%
  set_caption2("AUC values for XGBoost model", 3) %>%
  flextable_format
...
#   
# manual_lr <- tribble(~"name", ~"AUC_test", ~"AUC_train", ~"AUC_perc_performance_drop",
#                      'lr_topics', 0.563852, 0.5594643, 0,
#                      'lr_boruta', 0.58005028, 0.591204, 0
# )
# 
# auc_table_for_lr %>%
#   bind_rows(manual_lr) %>%
...
#   mutate(AUC_perc_performance_drop = (AUC_test-max(AUC_test))/max(AUC_test)) -> auc_table_for_lr
# 
# save(auc_table_for_lr, file=here('run_all_models_cache/auc_table_for_lr.Rdata'))
...
auc_table_for_lr %>%
  ...
  flextable::flextable() %>%
  set_caption2("AUC values for Logistic Regression model", 4) %>%
  flextable_format
```
...
On the plot \@ref(fig:lift-curve), a lift curve is presented. On the x-axis, the fraction of the *top* customers ranked by probability to buy for the second time is presented. On the y axis, a lift value for this quantile is shown. 
...
# Do appendixa
# save(output_lifts, file = here('data/output_lifts.Rdata'))
```
...
```{r lift-curve, fig.width = 6, fig.height = 4, fig.cap='Lift curve.'}
...
output_lifts %>%
  ...
  ggrepel::geom_text_repel(hjust = 'right', data =output_lifts %>%
                             filter(fraction_of_customers %in% (1-quantiles_to_show2)),
                           aes(label=sprintf('(%s%%: %s)', (1-quantiles_to_show2)*100, round(lift_over_randomly,1)))) +
  scale_y_continuous(breaks = c(1,5,10,15,20),limits = c(1, NA)) +
  scale_x_continuous(labels = function(x) sprintf('%.0f%%', x*100))+
  labs(x = "Fraction of the customers' ranking", y = "Lift")
...
\begin{equation}
score_f = 1-\frac{AUC_f-0.5}{AUC_{full}-0.5} (\#eq:vi-scale)
\end{equation}
...
To the left of the figure \@ref(fig:varimp-categories) the variables importances from the XGB model for the best variables (with included product categories) are presented.
The most important one is the transportation cost. Also, high importance scores are obtained by value of the payment and vanilla geolocation variables. 
...
source(here('feature_importance_custom.R'))
...
# ALL RAW
# features_groups_all <- models_list$product_categories$model$coefnames %>%
#   map(identity) %>%
#   setNames(models_list$product_categories$model$coefnames)
# 
# features_groups_all$if_second_order <- NULL
# 
# permutation_var_imp_cat <- calc_varimp_groups(models_list$product_categories$model, to_model_test, features_groups_all) %>% rename(var2 = feature_group, Overall = score)
# save(permutation_var_imp_cat, file = here('data/permutation_var_imp_cat.Rdata'))
load(file = here('data/permutation_var_imp_cat.Rdata'))

permutation_var_imp_cat %>%
...
# save(permutation_var_imp_cat_binned, file = here('data/permutation_var_imp_cat_binned.Rdata'))
load(file = here('data/permutation_var_imp_cat_binned.Rdata'))
...
<!-- ## Śmieci/do appendixa (TODO: spojrzeć na te kawałki i wykresy i raczej wyrzucić) -->

<!-- #### ROC curve  -->

<!-- dokładnie ta sama infromacja jest w lift curve. Pytanie czy jest sens zostawiać roc curve ?? -->
...
<!-- TODO: nie znalazłem właściwie uzasadnienia dla tych wykresów, myślę że spokojnie można wyrzucić bez utraty jakości pracy?? -->
...
<!-- Wszystkie hipotezy z każdego rozdziału gdziekolwiek postawione: -->
...
<!-- ### Lift analysis - advantages, usage -->
...
<!-- ### Skrawki z opisu lift analysis -->

<!-- TODO: Do zastanowienia czy zostawić -->

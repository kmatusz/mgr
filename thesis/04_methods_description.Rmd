...
#### text reviews analysis - 2 types, previous research on churn
...
Unfortunately, LDA approach was created with different purpose in mind. Typically reviews in the aspect of e-commerce are very short. @hong2010empirical showed that LDA is not able to find informative topics in short Twitter posts. Possible reason that LDA does not cope well is that assumption about a document being mixture of topics is false. Short texts probably comprise of very small amount of topics, usually only one [@hong2010empirical]. 


#### Movie group process

The drawbacks of LDA in setting of short texts were adressed by [@10.1145/2623330.2623715] . They used Gibbs Sampling algorithm for the Dirichlet Multinomial Mixture model, which is improvement over typical LDA. The authors show that this algorithm provides superior performance compared to basic LDA technique in the context of short texts. 



#### Aspect attention

More modern approaches to topic modeling were also developed recently. A recent milestone in the whole NLP field was introducing an efficient way to embed the words in a vector space while preserving its meaning, namely word2vec [@mikolov2013efficient]. On this basis, 

...
TODO: Opisać mapy, zastanowić się które wywalić

An aggregation to microregions is relatively coarse. Because of that, some of the statistics can be not reliable in the regions with a very small number of customers. That is why I have decided to remove from the map these microregions, in which number of customers was less than 5. 
...
Number of customers per 10 thousand population

```{r cache=T}
tm_shape(brazil_map) +
  tm_borders()+
  tm_shape(stats_per_microregion3) +
  tm_polygons(col = "no_customers_per_10000_pop",border.alpha = 0)
...
TODO: prawie nic nie widać, raczej do wyrzucenia
...
tm_shape(brazil_map) +
  tm_borders()+
  tm_shape(stats_per_microregion3) +
  tm_polygons(col = "mean_review_score",border.alpha = 0)

```


Percent of people that gave second order

```{r cache=T}
tm_shape(brazil_map) +
  tm_borders()+
  tm_shape(stats_per_microregion3) +
  tm_polygons(col = "percent_second_order",border.alpha = 0)
```
...
Mean transactions value

```{r cache=T}
tm_shape(brazil_map) +
  tm_borders()+
  tm_shape(stats_per_microregion3) +
  tm_polygons(col = "mean_payment_value",border.alpha = 0)
```

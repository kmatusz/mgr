...
knitr::opts_chunk$set(message = F, warning = F, echo=F, fig.width = 6, fig.height = 4)
```
...
## Modeling description (docelowo o wiele bardziej rozbudowane, tutaj tylko zalążek potrzebny do opisu rezultatów)
...
#### Methods (do przeklejenia do rozdziału methods description)
...
To assess the performance of Boruta algorithm one should however also look at the final accuracy of the model. The score using AUC on the test set is 0.646 - less than the model including all variables. This means that using Boruta algorithm did not bring an additional predictive power to the model. At the same time, this means that the variables indicating reviews topics are relevant for the model performance. 
...
The curve for the model with basic features is standing out of the others. However, the difference between 1st best and 2nd best models is not as clear - it looks like the better model has slightly better AUC, but this should be investigated more thoroughly. That is why I have used Kolmorogov-Smirnov test to check if the empirical distributions can come from the same probability distribution. I have run the test twice using 2 alternative hypotheses. First one with `H1: auc_best =/= auc_2nd_best`, and the second one: `H1: auc_best > auc_2nd_best`.  (TODO: rezultaty opisane w tekście i z kodu się rozjechały, sprawdzić dlaczego i czy zmienia konkluzje)

```{r}
ks.test(bootstrap_auc_best, bootstrap_auc_2nd_best)
ks.test(bootstrap_auc_best, bootstrap_auc_2nd_best,alternative = 'less')
```

The p-value for the first hypothesis is 0.0541. This means that with the level of significance 0.05 the performance of the models is indistinguishable. Using a more liberal level of significance of 0.1 one can say that the performance of the models is different.

P-value with 'greater' hypothesis is 0.02705. This means that at the levels of significance 0.1, 0.05 one can say that the performance of the first model is better than of the second one.

These two tests show that the results are ambiguous. Although there is some evidence that the performance of the product categories model is better, it cannot be said with a great certainty. One heuristic can come into help in deciding which model should be used in production setting is Occam's razor. The model with product categories has 21 variables, while the one with all variables included - 47. If there is no an important reason why the more complex approach should be used, the simpler is usually better. In this case, using simpler approach has the following advantages:

- Faster inference about the new customers
...
### Lift analysis - introduction (TODO: na tą chwilę skrawki tekstu, do poukładania)

Lift analysis is one of the ways to measure performance of marketing campaigns. Typically it is used in the scenario of A/B testing. In the case of churn prediction, a control group would not be given any encouragement to repeat the purchase, while testing group would receive some - for example an email containing discount code.
...
A/B testing is undoubtedly the most optimal way to test the performance of any machine learning model, at first the performance should be tested in an offline setting. This is because if the model performs particularly bad, the company is 

- co to lift
- normalne używanie - A/B testing
- dlaczego istotne dla marketingu
  - można targetować odpowiedni segment - tych którzy nie są oczywiści ale też tych którzy są prawdopodobni
  - można zestawić to z danemi z prowizji i zobaczyć który segment jest najbardziej zyskowny
  - cost analysis dla poszczególnych "segmentów" (kwantyli)

Lift is a typical measure of model performance. Typically it is calculated as a true positive rate obtained from a Machine Learning model on given threshold, divided by a proportion of the samples that are positive.
...
This information can be very valuable for the marketing campaigns led by the company. It can be used to enhance the performance of already existing marketing tools.

- For example, one can decide to not target the customers with the highest probability of buying the second time, as they would buy anyway. Another interesting insight that can be obtained from a well designed A/B testing experiment is the how the conversion rate of each of the customers group changes after performing some action, for example sending an encouraging email or giving a discount for the next purchase. 

- One strategy of marketing targeting based on lift information is targeting the customers that are the most susceptible to buying second time. This strategy can however generate unnecessary cost.
- A/B testing of each marketing strategy on each segment
- serves as a other baseline for marketing tools - instead of compute lift test group vs. control, compute lift test group vs. probability from the model
- select cutoffs for different types of targeting - for example don't do anything with top 1%, send encouraging email to top 10%, send discount to top 20%. Way to optimize costs of expensive targeting campaigns
...
### Lift analysis (TODO: opisać wykres i tabelkę)
...
0.033 of people have second order. This means that if we would send out the encouragement randomly, we would hit 0.033 percent of people that truly would do second order.
...
output_lifts %>%
  ...
  theme_minimal() + geom_point(data = output_lifts %>%
...
library(readxl)
brasil_cities_coords <- read_excel("data/brasil_cities_coords.xlsx")
...
It can be seen that the predictions form a very visible pattern in stripes. One possible explanation is the limitation of the model underlying XGBoost method, that is decision trees. The vanilla decision tree algorithm works by partitioning the feature space on a binary basis, and a typical output of that model on 2-d space are visible rectangles. And as XGBoost is simply stacked decision trees, the resulting partition pattern is a bit more complex, but still decision-tree-typical artifacts are visible. 

Another possible explanation comes from the work of ... (TODO: znaleźć pracę o wkładaniu surowych wartości long/lat do modelu, napisać paragraf). They noticed that ... 
...
Variable Attributions via Sequential Variable Conditioning - algorithm (?? cytowanie z paczki DALEX):

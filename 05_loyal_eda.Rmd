---
title: "Analysis of loyal customers"
output: html_document
---

In this notebook, basic information about "loyal" customers is analyzed. A customer is loyal if he returned to the shop after first order and bought something again. First, some data cleaning and preparation is needed: 
...
# for each order there can be multiple payments (installments). Aggregation to an order
order_payments %>%
  group_by(order_id) %>%
  summarise(payment_value = sum(payment_value)) -> order_payments_agg

# obtain the newest rating for each order 
order_reviews %>%
  group_by(order_id) %>%
  filter(review_creation_date == max(review_creation_date)) %>%
  select(order_id, review_score) -> ratings

# main join of relevant tables
orders %>%
  select(order_id,
         customer_id,
...
Checking number of items bought by each customer:
```{r}
orders_master %>%
  select(order_no) %>%
  group_by(order_no)%>%
...
We can check the behaviour of customers that bought at least one time, to see if it is possible to predict, which of the one-timers may be potentially interested in buying second time.

In other words, we can check the behaviour of loyal customer in the past, that is during
...
But for placing the third order when a customer made second order is 8.5%.
So, after making second buy the chances of longer loyalty grow. This proves that making the 
customer place second order is the hardest part in retention of the client.

Later data processing: select first order, with indication if particular customer bought also next time.
...
Distribution of review score placed after purchase - hypothesis is that low values of review don't lead to future buys - customer is discouraged to the shop.
...
why did the customer decide to buy again? This can also mean that customer loyalty is not so important in e-commerce sector!
